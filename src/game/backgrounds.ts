...
function generateBackground(): HTMLCanvasElement {
  ...
  for (let bx = 0; bx < GAME_CONFIG.canvasWidth; bx += 40) {
    ...
    for (let wy = GAME_CONFIG.groundY - 55 - bh; wy < GAME_CONFIG.groundY - 60; wy += 8) {
      for (let wx = bx + 4; wx < bx + 32; wx += 10) {
        if (Math.random() > 0.4) {
          ctx.fillStyle = Math.random() > 0.5 ? '#ffcc44' : '#44ccff'
  ...
  ctx.setLineDash([4, 8])
  ctx.beginPath()
  ctx.moveTo(0, GAME_CONFIG.groundY - 2)
  ctx.lineTo(GAME_CONFIG.canvasWidth, GAME_CONFIG.groundY - 2)
  ctx.stroke()
  ctx.setLineDash([])
  ctx.globalAlpha = 1
