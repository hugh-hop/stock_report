import { Mecha, GAME_CONFIG } from './config'

export function updatePhysics(mecha: Mecha): Mecha {
  const m = { ...mecha }
...
export function applyKnockback(mecha: Mecha, direction: number, force: number): Mecha {
  const m = { ...mecha }
  m.velocityX = direction * force
  m.velocityY = -2
  return m
}
